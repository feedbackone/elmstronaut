...
async function compileElm(filePath: string, isAstroDevMode: boolean): Promise<string> {
  // Example
  ...
  const elmFileRelativePath = normalizedFilePath.replace(`${elmDir}${path.sep}`, "")
  ...
  await new Promise<void>((resolve, reject) => {
    const elmMake = spawn(
      elmExecutable,
      ["make", elmFileRelativePath, "--output", outputFilePath, isAstroDevMode ? "" : "--optimize"]
