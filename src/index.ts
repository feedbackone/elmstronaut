...
import { BOOTSTRAP_SCRIPT_PATH, ELMSTRONAUT_D_TS_PATH, PACKAGE_ROOT_DIR } from "./const.js"
...
export default function elmstronaut(): AstroIntegration {
  return {
    name: "elmstronaut",
    hooks: {
      "astro:config:setup": async ({ addRenderer, command, injectScript, updateConfig }) => {
        // Inject the bootstrap script.
        // We need this to properly handle `window.onElmInit` callback definition.
        injectScript("head-inline", readFileSync(BOOTSTRAP_SCRIPT_PATH, "utf-8"))
